...
class LteUePhy : public LtePhy
{
  ...
  public:
     ...
     */
    virtual void ReceivePsdchSdRsrp(Ptr<Packet> p,
     ...
     * \param [in] sdRsrp
     */
    typedef void (*SdRsrpTracedCallback)(uint16_t rnti,
                                         uint64_t relayId,
  ...
  private:
     ...
     */
    virtual void DoSendSlMacPdu(Ptr<Packet> p,
                                LteUePhySapProvider::TransmitSlPhySduParameters params);
    /**
     ...
     */
    std::list<LteUePhySapProvider::TransmitSlPhySduParameters> GetSlPhyParameters(
        Ptr<PacketBurst> pb);
    ...
    struct RxPoolInfo
    {
        ...
        SidelinkCommResourcePool::SubframeInfo m_currentScPeriod; ///< start of current period
        SidelinkCommResourcePool::SubframeInfo m_nextScPeriod;    ///< start of next period
        std::map<uint16_t, SidelinkGrantInfo>
            m_currentGrants; ///< Current Sidelink communication grants
    };
    ...
    {
        uint16_t m_rnti;    ///< RNTI
        uint8_t m_resPsdch; ///< m_resPsdch A randomly chosen resource index from the PSDCH resource
                            ///< pool
    };
     ...
     * The index is the simulation time and the elements are the SyncRef identifiers (SLSSID and
     * offset)
     */
    ...
    /**
     * Time period for searching/scanning to detect available SyncRefs (supporting SyncRef
     * selection)
     */
    ...
    /**
     * Time period for taking S-RSRP samples of the selected SyncRef (supporting SLSS transmission
     * decision)
     */
    ...
    /**
     * True if a SyncRef selection is in progress and the UE is performing the SyncRef
     * search/scanning
     */
     ...
     * either of the detected SyncRefs (measurement sub-process) or the selected one (evaluation
     * sub-process)
     */
    bool m_ueSlssMeasurementInProgress;
    /**
     * Number of S-RSRP measurement periods already performed in SyncRef selection process in
     * progress (1 = measurement, 2 = measurement + evaluation)
     */
     ...
     * 2. Evaluation: collect the S-RSRP of the selected SyncRef (if any) to determine if the UE
     * needs to become itself a SyncRef and start transmitting SLSS
     *
     ...
     *
     * \param endOfPrevious This indicates after which sub-process the SyncRef selection process
     * ended: 0 if it ended after scanning, 1 if it ended after measurement, or 2 if it ends after
     * evaluation
     */
     ...
     * The RRC instructs the PHY to synchronize to a given SyncRef and apply the corresponding
     * change of timing
     *
