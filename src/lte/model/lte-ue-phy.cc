...
class UeMemberLteUePhySapProvider : public LteUePhySapProvider
{
  public:
    ...
    void SendSlMacPdu(Ptr<Packet> p, TransmitSlPhySduParameters params) override;
    ...
    static TypeId tid =
        TypeId("ns3::LteUePhy")
            ...
            .AddAttribute("RsrpUeMeasThreshold",
                          ...
                          DoubleValue(-1000.0), // to avoid changing the default behavior, make it
                                                // low so that it acts as if it was not used
            ...
            .AddAttribute("UeSlssInterScanningPeriodMax",
                          "The upper bound of the uniform random variable for the interval between "
                          "SyncRef selection processes",
                          TimeValue(MilliSeconds(2000)),
                          MakeTimeAccessor(&LteUePhy::SetUeSlssInterScanningPeriodMax),
                          MakeTimeChecker())
            .AddAttribute("UeSlssInterScanningPeriodMin",
                          "The lower bound of the uniform random variable for the interval between "
                          "SyncRef selection processes",
                          TimeValue(MilliSeconds(2000)),
            ...
            .AddAttribute("UeSlssMeasurementPeriod",
                          "How long the UE will perform SLSS L1 measurements for SyncRef selection "
                          "(measurement)",
                          TimeValue(MilliSeconds(400)),
                          MakeTimeAccessor(&LteUePhy::m_ueSlssMeasurementPeriod),
                          MakeTimeChecker())
            .AddAttribute("UeSlssEvaluationPeriod",
                          "How long the UE will perform SLSS L1 measurements to determine "
            ...
            .AddAttribute("NSamplesSrsrpMeas",
                          "The maximum number of samples to take during SLSS L1 measurements for "
                          "each SyncRef",
                          UintegerValue(4),
                          MakeUintegerAccessor(&LteUePhy::m_nSamplesSrsrpMeas),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute(
                "UeRandomInitialSubframeIndication",
                "If True, the first frame and subframe values (beginning of the simulation) are "
                "chosen randomly, if False they are fixed to 1,1 respectively",
                BooleanValue(false),
...
LteUePhy::GetSlPhyParameters(Ptr<PacketBurst> pb)
{
    ...
    if (!m_packetParamsQueue.at(0).empty())
    {
        ...
        if (sciHeader.GetGroupDstId() == ((*it) & 0xFF))
        {
            ...
            else
            {
                ...
                if (grantIt == poolIt->m_currentGrants.end())
                {
                    ...
                    // insert grant
                    poolIt->m_currentGrants.insert(
                        std::pair<uint16_t, SidelinkGrantInfo>(tag.GetRnti(), txInfo));
                } // else it should be the retransmission and the data should be the same...add
                  // check
                else
                {
...
LteUePhy::PhyPsbchPduReceived(Ptr<Packet> p, uint16_t slssid)
{
    ...
    if (m_ueSlssScanningInProgress)
    {
        ...
        m_detectedMibSl.insert(
            std::pair<std::pair<uint16_t, uint16_t>, LteRrcSap::MasterInformationBlockSL>(
...
LteUePhy::ReceiveLteControlMessageList(std::list<Ptr<LteControlMessage>> msgList)
{
    ...
    for (auto it = msgList.begin(); it != msgList.end(); it++)
    {
        ...
        if (msg->GetMessageType() == LteControlMessage::DL_DCI)
        {
            ...
            // Since we are receiving packets in the downlink, we will need to send an HARQ
            // feedback, let's tell the MAC about it now so it does not schedule sidelink
            // transmission
            m_uePhySapUser->NotifyUlTransmission();
        }
...
LteUePhy::SubframeIndication(uint32_t frameNo, uint32_t subframeNo)
{
    ...
    // If a change of timing (resynchronization) was requested before, do the change of frameNo and
    // subframeNo if possible
    ...
    if (m_ulConfigured)
    {
        ...
        if (m_slTxPoolInfo.m_pool)
        {
            ...
            if (m_slTxPoolInfo.m_nextScPeriod.frameNo == 0)
            {
                ...
                m_slTxPoolInfo.m_nextScPeriod =
                    m_slTxPoolInfo.m_pool->GetNextScPeriod(frameNo - 1, subframeNo - 1);
            ...
            if (frameNo == m_slTxPoolInfo.m_nextScPeriod.frameNo &&
                subframeNo == m_slTxPoolInfo.m_nextScPeriod.subframeNo)
            {
                ...
                m_slTxPoolInfo.m_nextScPeriod.frameNo++;
                m_slTxPoolInfo.m_nextScPeriod.subframeNo++;
                NS_LOG_INFO("Starting new SC period for TX pool "
                            << ". Next period at " << m_slTxPoolInfo.m_nextScPeriod.frameNo << "/"
                            << m_slTxPoolInfo.m_nextScPeriod.subframeNo);
            ...
            while (grantIt != it->m_currentGrants.end())
            {
                ...
                if (grantIt->second.m_grantReceived)
                {
                    ...
                    SidelinkCommResourcePool::SubframeInfo tmp =
                        it->m_pool->GetCurrentScPeriod(frameNo - 1, subframeNo - 1);
                    grantIt->second.m_psschTx =
                        it->m_pool->GetPsschTransmissions(tmp,
                                                          grantIt->second.m_grant.m_trp,
                                                          grantIt->second.m_grant.m_rbStart,
                                                          grantIt->second.m_grant.m_rbLen);

                    for (rxIt = grantIt->second.m_psschTx.begin();
                         rxIt != grantIt->second.m_psschTx.end();
                         rxIt++)
                    {
                        ...
                        NS_LOG_INFO("Subframe Rx" << rxIt->subframe.frameNo << "/"
                                                  << rxIt->subframe.subframeNo
                                                  << ": rbStart=" << (uint32_t)rxIt->rbStart
                                                  << ", rbLen=" << (uint32_t)rxIt->nbRb);
                    }
                ...
                {
                    NS_LOG_DEBUG(frameNo << "/" << subframeNo << " RNTI=" << m_rnti
                                         << " next pssch at " << (*rxIt).subframe.frameNo << "/"
                                         << (*rxIt).subframe.subframeNo);
                }

                if (rxIt != grantIt->second.m_psschTx.end() &&
                    (*rxIt).subframe.frameNo == frameNo &&
                    (*rxIt).subframe.subframeNo == subframeNo)
                {
                    // reception
                    NS_LOG_INFO("Expecting PSSCH reception RB "
                                << (uint16_t)rxIt->rbStart << " to "
                                << (uint16_t)(rxIt->rbStart + rxIt->nbRb - 1));
        ...
        if (rbMask.empty())
        {
            // we do not have uplink data to send, make sure that if we are sending sidelink
            // messages that there is no control messages also being sent (like DL_HARQ or CQI
            // feedbacks)
            ...
            NS_ASSERT_MSG(!pb || ctrlMsg.empty(),
                          "Trying to send sidelink along with UL control messages");
        } // end of if (rbMask.size () == 0)

        if (!rbMask.empty() || !ctrlMsg.empty())
        {
        ...
        else
        {
            ...
            if (!m_waitingNextScPeriod)
            {
                ...
                NS_LOG_LOGIC(this << " UE - start slot for PSSCH + PSCCH - RNTI " << m_rnti
                                  << " CELLID " << m_cellId);
                ...
                if (pb)
                {
                    ...
                    NS_ASSERT_MSG(ctrlMsg.empty(),
                                  "Should not have control messages while sending SL packet burst");
                    ...
                    case LteUePhySapProvider::TransmitSlPhySduParameters::PSBCH:
                        ...
                        NS_ASSERT_MSG(params.size() == 1,
                                      "Found more than one packet in PSBCH burst");

                        if (m_enableUplinkPowerControl)
                        {
                            m_txPower = m_powerControl->GetPscchTxPower(
                                slRb); // what's the power control for PSBCH?
                        }
                    ...
                    case LteUePhySapProvider::TransmitSlPhySduParameters::PSCCH:
                        ...
                        NS_ASSERT_MSG(params.size() == 1,
                                      "Found more than one packet in PSCCH burst");
                        ...
                        if (m_ueSlssScanningInProgress)
                        {
                            NS_LOG_LOGIC(this << "trying to do a PSCCH transmission while there is "
                                                 "a scanning in progress... Ignoring transmission");
                        }
                        else if (m_ueSlssMeasurementsSched.find(
                                     Simulator::Now().GetMilliSeconds()) !=
                                 m_ueSlssMeasurementsSched.end()) // Measurement in this subframe
                        {
                            NS_LOG_LOGIC("trying to do a PSCCH transmission while measuring S-RSRP "
                                         "in the same subframe... Ignoring transmission");
                        }
                        else
                        {
                            SetSubChannelsForTransmission(slRb);
                    ...
                    case LteUePhySapProvider::TransmitSlPhySduParameters::PSSCH:
                        // NS_ASSERT (ctrlMsg.size () == 0); //(In the future we can have PSSCH and
                        // MIB-SL in the same subframe)
                        NS_LOG_LOGIC(this << " UE - start TX PSSCH");
                        NS_LOG_DEBUG(this << " TX Burst containing " << pb->GetNPackets()
                                          << " packets");
                        ...
                        {
                            NS_LOG_LOGIC(this
                                         << " trying to do a PSSCH transmission while there is a "
                                            "scanning in progress... Ignoring transmission");
                        }
                        else if (m_ueSlssMeasurementsSched.find(
                                     Simulator::Now().GetMilliSeconds()) !=
                                 m_ueSlssMeasurementsSched.end())
                        {
                            NS_LOG_LOGIC("trying to do a PSSCH transmission while measuring S-RSRP "
                                         "in the same subframe... Ignoring transmission");
                        }
                        else
                        {
                            SetSubChannelsForTransmission(slRb);
                            m_uplinkSpectrumPhy->StartTxSlDataFrame(pb,
                                                                    UL_DATA_DURATION,
                                                                    (*paramIt).dstId);
                        }
                        break;
                    case LteUePhySapProvider::TransmitSlPhySduParameters::PSDCH:
                        ...
                        //
                        m_uplinkSpectrumPhy->StartTxSlDiscFrame(pb,
                                                                (*paramIt).resNo,
                                                                (*paramIt).rv,
                                                                UL_DATA_DURATION);
            ...
            {
                NS_LOG_LOGIC(
                    this << " the UE changed of timing and it is waiting for the start of a new SC "
                            "period using the new timing... Delaying transmissions ");
            }
        }
...
LteUePhy::DoConfigureUplink(uint32_t ulEarfcn, uint16_t ulBandwidth)
{
    ...
    if ((m_ulEarfcn != ulEarfcn) || (m_ulBandwidth != ulBandwidth) || (!m_ulConfigured))
    {
        ...
        if (m_sidelinkSpectrumPhy)
        {
            ...
            m_slNoisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity(m_ulEarfcn,
                                                                                   m_ulBandwidth,
                                                                                   m_noiseFigure);
...
LteUePhy::SwitchToState(State newState)
{
    ...
    m_state = newState;
    NS_LOG_INFO("cellId=" << m_cellId << " rnti=" << m_rnti << " UePhy " << ToString(oldState)
                          << " --> " << ToString(newState));
...
LteUePhy::DoSetSlDiscRxPools(std::list<Ptr<SidelinkRxDiscResourcePool>> pools)
{
    ...
    for (poolIt = pools.begin(); poolIt != pools.end(); poolIt++)
    {
        ...
        for (currentPoolIt = m_discRxPools.begin(); currentPoolIt != m_discRxPools.end() && !found;
             currentPoolIt++)
        {
...
LteUePhy::DoSetSlCommRxPools(std::list<Ptr<SidelinkRxCommResourcePool>> pools)
{
    ...
    for (poolIt = pools.begin(); poolIt != pools.end(); poolIt++)
    {
        ...
        for (currentPoolIt = m_sidelinkRxPools.begin();
             currentPoolIt != m_sidelinkRxPools.end() && !found;
             currentPoolIt++)
        {
            if (*poolIt == currentPoolIt->m_pool)
            {
...
LteUePhy::ReceiveSlss(uint16_t slssid, Ptr<SpectrumValue> p)
{
    ...
    if (m_ueSlssScanningInProgress || m_ueSlssMeasurementInProgress)
    {
        ...
        for (itPi = p->ConstValuesBegin(); itPi != p->ConstValuesEnd(); itPi++)
        {
            ...
            {
                double powerTxW =
                    ((*itPi) * 180000.0) / 12.0; // convert PSD [W/Hz] to linear power [W]
                sum += powerTxW;
                nRB++;
            }
        }
        ...
        if (itMeasMap == m_ueSlssMeasurementsMap.end()) // First entry
        {
            ...
            if (m_ueSlssScanningInProgress)
            {
                ...
                m_ueSlssDetectionMap.insert(
                    std::pair<std::pair<uint16_t, uint16_t>, UeSlssMeasurementsElement>(
            ...
            else if (m_ueSlssMeasurementInProgress)
            {
                ...
                if (itDetectionMap != m_ueSlssDetectionMap.end())
                {
                    ...
                    m_ueSlssMeasurementsMap.insert(
                        std::pair<std::pair<uint16_t, uint16_t>, UeSlssMeasurementsElement>(
                            std::pair<uint16_t, uint16_t>(slssid, offset),
                            newEl));
                }
                ...
                {
                    NS_LOG_LOGIC(
                        "SyncRef was not detected during SyncRef search/scanning... Ignoring");
                }
            }
        }
        else
    ...
    {
        NS_LOG_LOGIC(this << " The UE is not currently performing SyncRef scanning or S-RSRP "
                             "measurement... Ignoring");
    }
}
...
LteUePhy::EndSlssScanning()
{
    ...
    for (itDetectionMap = m_ueSlssDetectionMap.begin();
         itDetectionMap != m_ueSlssDetectionMap.end();
         itDetectionMap++)
    {
        NS_LOG_LOGIC(
    ...
    for (itDetectionMap = m_ueSlssDetectionMap.begin();
         ...
         itDetectionMap++)
    {
        tmp.insert(std::pair<double, std::pair<uint16_t, uint16_t>>(
            itDetectionMap->second.srsrpSum / itDetectionMap->second.srsrpNum,
            itDetectionMap->first));
    }
    while (tmp.size() > 6)
    {
        NS_LOG_LOGIC(
            "The UE detected more than 6 SyncRefs... Removing lowest S-RSRP SyncRef: SLSSID"
            << tmp.begin()->second.first << "ofset " << tmp.begin()->second.second << "S-RSRP "
            << tmp.begin()->first);
    ...
    for (itTmp = tmp.begin(); itTmp != tmp.end(); itTmp++)
    {
        ...
        if (itDetectionMapTwo != m_ueSlssDetectionMap.end())
        {
            ret.insert(std::pair<std::pair<uint16_t, uint16_t>, UeSlssMeasurementsElement>(
                itDetectionMapTwo->first,
                itDetectionMapTwo->second));
        }
    }

    m_ueSlssDetectionMap = ret;    // It contains now only the 6 SyncRefs with higher S-RSRP
    m_ueSlssMeasurementsMap = ret; // It contains now only the 6 SyncRefs with higher S-RSRP (we use
                                   // the S-RSRP measurements during scanning as first measurements)

    uint32_t nDetectedSyncRef = m_ueSlssDetectionMap.size();

    if (nDetectedSyncRef > 0)
    {
        NS_LOG_LOGIC("At least one SyncRef detected, creating measurement schedule and starting "
                     "measurement sub-process");
        // Create measurement schedule
        std::map<std::pair<uint16_t, uint16_t>, UeSlssMeasurementsElement>::iterator itMeasMap;
        for (itMeasMap = m_ueSlssMeasurementsMap.begin();
             itMeasMap != m_ueSlssMeasurementsMap.end();
             itMeasMap++)
        {
            ...
            {
                t = Simulator::Now().GetMilliSeconds() +
                    (40 - currOffset + itMeasMap->first.second);
            }
            uint16_t count = 1;
            while (t < (Simulator::Now().GetMilliSeconds() +
                        m_ueSlssMeasurementPeriod.GetMilliSeconds() - 40))
            {
                NS_LOG_INFO("UE RNTI " << m_rnti << " will measure S-RSRP of SyncRef SLSSID "
                                       << itMeasMap->first.first << " offset "
                                       << itMeasMap->first.second << " at t:" << t << " ms");
                m_ueSlssMeasurementsSched.insert(
                    std::pair<int64_t, std::pair<uint16_t, uint16_t>>(t, itMeasMap->first));
                count++;
...
LteUePhy::StartSlssMeasurements(uint64_t slssid, uint16_t offset)
{
    ...
    if (slssid == 0) // Measurement
    {
        ...
        NS_LOG_LOGIC("Starting S-RSRP measurement corresponding to the measurement sub-process... "
                     "Report happening in "
                     << t);
    }
    else // Evaluation
    {
        ...
        NS_LOG_LOGIC("Starting S-RSRP measurement corresponding to the evaluation sub-process... "
                     "Report happening in "
                     << t);
    }
...
LteUePhy::ReportSlssMeasurements(uint64_t slssid, uint16_t offset)
{
    ...
    {
        NS_LOG_LOGIC("End of S-RSRP measurement corresponding to the measurement sub-process... "
                     "Reporting L1 filtered S-RSRP values of detected SyncRefs");

        for (it = m_ueSlssMeasurementsMap.begin(); it != m_ueSlssMeasurementsMap.end(); it++)
        {
            // L1 filtering: linear average
            double avg_s_rsrp_W =
                (*it).second.srsrpSum / static_cast<double>((*it).second.srsrpNum);
            // The stored values are in W, the report to the RRC should be in dBm
            double avg_s_rsrp_dBm = 10 * log10(1000 * (avg_s_rsrp_W));

            NS_LOG_INFO("UE RNTI " << m_rnti << " report SyncRef with SLSSID " << (*it).first.first
                                   << " offset " << (*it).first.second << " L1 filtered S-RSRP "
                                   << avg_s_rsrp_dBm << " from " << (double)(*it).second.srsrpNum
                                   << " samples");
    ...
    else // Report only of the selected SyncRef
    {
        ...
        NS_LOG_LOGIC("Reporting L1 filtered S-RSRP values of the SyncRef SLSSID "
        ...
        if (it != m_ueSlssMeasurementsMap.end())
        {
            ...
            NS_LOG_INFO(Simulator::Now().GetMilliSeconds()
                        << " UE RNTI " << m_rnti << " Report SyncRef with SLSSID "
                        << (*it).first.first << " offset " << (*it).first.second
                        << " L1 filtered S-RSRP " << avg_s_rsrp_dBm << " from "
                        << (double)(*it).second.srsrpNum << " samples");
    ...
    // Schedule the start of the measurement period for evaluation of selected SyncRef if
    // appropriated
    m_currNMeasPeriods++;
    if (m_currNMeasPeriods == 1 && m_resyncRequested)
    {
        NS_LOG_LOGIC(
            "The measurement sub-process ended and RRC selected a SyncRef for (re)synchronization");

        // Schedule the measurement for evaluation of the selected SyncRef for initiation/cease of
        // SlSS transmission
        NS_LOG_INFO("UE RNTI " << m_rnti
                               << " will start evaluation of selected SyncRef with SLSSID "
                               << m_resyncParams.slssid << " offset" << m_resyncParams.offset);
        Simulator::ScheduleNow(&LteUePhy::StartSlssMeasurements,
                               this,
                               m_resyncParams.slssid,
                               m_resyncParams.offset);
        ...
        while (t < (Simulator::Now().GetMilliSeconds() +
                    m_ueSlssMeasurementPeriod.GetMilliSeconds() - 40))
        {
            NS_LOG_INFO("UE RNTI " << m_rnti << " will measure SyncRef with SLSSID"
                                   << m_resyncParams.slssid << " offset " << m_resyncParams.offset
                                   << " at t:" << t << " ms");
            m_ueSlssMeasurementsSched.insert(std::pair<int64_t, std::pair<uint16_t, uint16_t>>(
                t,
                std::pair<uint16_t, uint16_t>(m_resyncParams.slssid, m_resyncParams.offset)));
            count++;
            if (count > m_nSamplesSrsrpMeas)
            {
                break;
    ...
    {
        // End of the selection+evaluation process, reinitialize variables for next process and
        // schedule it
        m_ueSlssDetectionMap.clear();

        if (m_currNMeasPeriods == 1)
        {
            NS_LOG_LOGIC("The measurement sub-process ended and RRC did not selected a SyncRef... "
                         "Ending SyncRef selection process");
...
LteUePhy::ScheduleNextSyncRefReselection(uint16_t endOfPrevious)
{
    ...
    NS_LOG_DEBUG("t_nextProcess " << t_nextProcess << " m_ueSlssScanningPeriod.GetMilliSeconds "
                                  << m_ueSlssScanningPeriod.GetMilliSeconds());
    ...
    case 2:
        ...
        t_nextProcess = t_nextProcess - (m_ueSlssScanningPeriod.GetMilliSeconds() +
                                         m_ueSlssMeasurementPeriod.GetMilliSeconds() +
                                         m_ueSlssEvaluationPeriod.GetMilliSeconds());
        break;
    }
    ...
    {
        NS_LOG_LOGIC("Attempted to schedule the next SyncRef selection process for a period larger "
                     "than 20 s... Scheduling it for 20 s");
        t_nextProcess = 20000;
    }
    ...
    if (t_nextProcess <= 0)
    {
        NS_LOG_LOGIC("Attempted to schedule the next SyncRef selection process for the past... "
                     "Scheduling it for next subframe");
        t_nextProcess = 1;
    }
    NS_LOG_INFO("UE RNTI " << m_rnti << " will start the next SyncRef selection process in t: "
                           << t_nextProcess << " ms");
...
LteUePhy::ChangeOfTiming(uint32_t frameNo, uint32_t subframeNo)
{
    ...
    if (m_slTxPoolInfo.m_pool)
    {
        ...
        if (((frameNo == m_slTxPoolInfo.m_nextScPeriod.frameNo &&
             ...
             m_slTxPoolInfo.m_nextScPeriod.frameNo == 0))
        {
            NS_LOG_LOGIC("The current subframe corresponds to the start of a new Sidelink "
                         "communication period... Applying the change of timing");

            // Calculate the new subframe indication
            SidelinkCommResourcePool::SubframeInfo mibTime;
            NS_LOG_INFO("mibTime frame/subframe = " << m_resyncParams.syncRefMib.directFrameNo
                                                    << "/"
                                                    << m_resyncParams.syncRefMib.directSubframeNo);
            ...
            NS_LOG_INFO("rxSubframe frame/subframe = " << m_resyncParams.rxSubframe.frameNo << "/"
                                                       << m_resyncParams.rxSubframe.subframeNo);
            ...
            m_resyncRequested = false;
            NS_LOG_INFO("UE RNTI " << m_rnti << " has a TxPool and changed the Subframe Indication"
                                   << " from " << frameNo << "/" << subframeNo << " to "
                                   << m_resyncParams.newSubframe.frameNo << "/"
                                   << m_resyncParams.newSubframe.subframeNo);
            ...
            m_slTxPoolInfo.m_nextScPeriod =
                m_slTxPoolInfo.m_pool->GetNextScPeriod(m_slTxPoolInfo.m_currentScPeriod.frameNo,
                                                       m_slTxPoolInfo.m_currentScPeriod.subframeNo);
            // adjust because scheduler starts with frame/subframe = 1
            m_slTxPoolInfo.m_nextScPeriod.frameNo++;
            m_slTxPoolInfo.m_nextScPeriod.subframeNo++;
            NS_LOG_INFO("UE RNTI " << m_rnti
                                   << " Next Sidelink communication Tx period at frame/subframe: "
                                   << m_slTxPoolInfo.m_nextScPeriod.frameNo << "/"
                                   << m_slTxPoolInfo.m_nextScPeriod.subframeNo);
        ...
        { // Delay the change of Timing
            NS_LOG_LOGIC("The current subframe does not correspond to the start of a new Sidelink "
                         "communication period... Delaying the change of timing");
            return false;
        }
    }
    else
    {
        ...
        NS_LOG_LOGIC("The UE is not currently transmitting Sidelink communication... Applying the "
                     "change of timing");
...
LteUePhy::DoSynchronizeToSyncRef(LteSlSyncParams synchParams)
{
    ...
    NS_LOG_INFO("Synchronizing to SyncRef SLSSSID " << synchParams.slssid << " offset "
                                                    << synchParams.offset);
    // NS_LOG_INFO ("The estimated CURRENT subframe indication of the SyncRef (frame/subframe): " <<
    // synchParams.newFrameNo << "/" << synchParams.newSubframeNo);
    NS_LOG_INFO("The CURRENT subframe indication of this UE (frame/subframe): "
...
LteUePhy::ReceivePsdchSdRsrp(Ptr<Packet> p, Ptr<SpectrumValue> psd, const std::vector<int>& rbs)
{
        ...
        msgType == LteSlDiscHeader::DISC_RELAY_RESPONSE)
    {
        ...
        NS_LOG_INFO(this << " UE RNTI " << m_rnti << " Rx relay discovery message from Relay UE Id "
                         << relayUeId << " Service Code " << serviceCode << " SD-RSRP (dBm) "
                         << sd_rsrp_dBm << " nRB " << nRB);
        ...
        if (itMeasMap == m_ueSdRsrpMeasurementsMap.end())
        {
            ...
            m_ueSdRsrpMeasurementsMap.insert(
                std::pair<std::pair<uint64_t, uint32_t>, UeSdRsrpMeasurementsElement>(
                    std::pair<uint64_t, uint32_t>(relayUeId, serviceCode),
                    newEl));
        }
        else
        {
...
LteUePhy::DoEnableUeSdRsrpMeasurements()
{
    ...
    {
        Simulator::Schedule(GetUeSdRsrpMeasurementsFilterPeriod(),
...
LteUePhy::ReportUeSdRsrpMeasurements()
{
    ...
    if (m_ueSdRsrpMeasurementsEnabled)
    {
        ...
        for (itMeasMap = m_ueSdRsrpMeasurementsMap.begin();
             itMeasMap != m_ueSdRsrpMeasurementsMap.end();
             itMeasMap++)
        {
            // L1 filtering: linear average
            double avg_sd_rsrp_W =
                (*itMeasMap).second.sdRsrpSum / static_cast<double>((*itMeasMap).second.sdRsrpNum);
            // The stored values are in W, the report to the RRC should be in dBm
            double avg_sd_rsrp_dBm = 10 * log10(1000 * (avg_sd_rsrp_W));

            NS_LOG_INFO(this << " Relay UE Id " << (*itMeasMap).first.first << "Service Code "
                             << (*itMeasMap).first.second << " averaged SD-RSRP (dBm) "
                             << avg_sd_rsrp_dBm << " number of measurements "
                             << (*itMeasMap).second.sdRsrpNum);
            ...
            m_reportUeSdRsrpMeasurements(m_rnti,
                                         (*itMeasMap).first.first,
                                         (*itMeasMap).first.second,
                                         avg_sd_rsrp_dBm);
        }
        // Report to RRC
